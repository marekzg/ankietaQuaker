...
class HomeController extends Controller
{
    ...
    {
        $rankingUsers = DB::table('isgroupUsers')->select('isgroupUsers.id', 'isgroupUsers.imie', 'isgroupUsers.foto', 'isgroupUsers.ocena')->orderBy('ocena', 'desc')->limit(10)->get();
    ...
    public function randomUserWin(int $start=0)
    {
        $tokenUsed = DB::table('tokenUser')->where('tokenUser.status', 1)->count();
        $tokenWin = DB::table('tokenUser')->where('tokenUser.status', 1)->inRandomOrder()->first();
        $listTokens = DB::table('tokenUser')->where('tokenUser.status', 1)->get();

        return view('login.randomTokenWin', [
            'start' => $start,
            'tokenUsed' => $tokenUsed,
            'tokenWin' => $tokenWin,
            'listTokensFree' => $listTokens,
        ]);
    }

    public function randomOrganization(int $start=0)
    {
        $organizationsCount = DB::table('questions')->where('questions.question3', '!=', '')->count();
        $randomOrganization = DB::table('questions')->where('questions.question3', '!=', '')->inRandomOrder()->first();
        $organizations = DB::table('questions')
        ->select('questions.tokenUser_token', 'questions.question3')
        ->where('questions.question3', '!=', '')
        ->orderBy('updated_at', 'asc')->get();


        return view('login.randomOrganization', [
            'start' => $start,
